# Public-key Cryptography in Julia
Primality and public-key cryptography routines in Julia
...
Clifford Cocks' original proposal for public-key cryptography. Originally a classified memorandum at GCHQ in 1973.
...
```
Be cautious of Carmichael pseudoprimes.
```
...
The General Number Field Sieve is not really Julia-friendly.

If you want to factor numbers used for public keys, you should not be doing it in
Julia and should expect to rent time at a data center.
...
* `isOdd(x)` and `isEven(x)`
* `power(a, e)`
* `powerMod(a, e, n)`
* `perfectPower(n)`
* `isPrimeMR(n, k)` &mdash; Miller-Rabin
* `isPrimeSS(n, k)` &mdash; Solovay-Strassen
* `isPrimeLS(n)` &mdash; Lucas (probable prime)
* `isPrimeF(n)` &mdash; Fermat
* `isPrimeBPSW(n)` &mdash; Baillie-Pomerance-Selfridge-Wagstaff
* `isPrime(n, k) = isPrimeMR(n, k)` (by default)
* `randomPrime(low, high, k)`
* `safePrime(low, high, k)`
* `rabinPrime(low, high)`
* `extendedGCD(a, b)`
* `gcd(a, b)`
* `lcm(a, b)`
